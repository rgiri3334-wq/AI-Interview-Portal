...
const BUNDLES = [
  {
    name: "Startup Culture Bundle",
    icon: <Activity size={16} />,
    contexts: [
      { text: "We operate in a very fast-paced startup environment. Emphasize agility.", priority: "HIGH" },
      { text: "We use a lot of open source technologies. Ask about open source contributions.", priority: "MEDIUM" }
    ]
  },
  {
    name: "Enterprise Mode Bundle",
    icon: <ShieldOff size={16} />,
    contexts: [
      { text: "Security and scale are our top priorities. Probe for enterprise experience.", priority: "CRITICAL" },
      { text: "Compliance with GDPR and SOC2 is mandatory. Ask if they know the basics.", priority: "HIGH" }
    ]
  }
];
...
export default function GlobalContextFeed({ companyContext, handleSaveCompanyContext, showToast }) {
  ...
  const handleInjectBundle = (bundle) => {
    const newContexts = bundle.contexts.map((c, i) => ({
    ...
    handleSaveCompanyContext([...newContexts, ...companyContext]);
    showToast(`${bundle.name} injected!`);
  };
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="space-y-6 max-w-[1200px] mx-auto">
      ...
      {/* Top Section: Injector & Bundles */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        
        {/* Main Injector */}
        <div className="lg:col-span-2 bg-white border border-slate-100 rounded-3xl p-8 shadow-[0_8px_30px_rgb(0,0,0,0.04)] relative overflow-hidden">
        ...
        {/* Bundles */}
        <div className="lg:col-span-1 bg-slate-900 border border-slate-800 rounded-3xl p-8 shadow-xl text-white relative overflow-hidden group">
          <div className="absolute right-0 bottom-0 w-32 h-32 bg-red-600 rounded-full blur-[80px] opacity-20 group-hover:opacity-40 transition-opacity" />
          <h3 className="text-xl font-extrabold mb-6 flex items-center tracking-tight">
            <PackagePlus size={20} className="text-red-500 mr-3" />
            Quick Bundles
          </h3>
          <div className="space-y-4 relative z-10">
            {BUNDLES.map((b, i) => (
              <div key={i} className="bg-slate-800/80 backdrop-blur border border-slate-700 rounded-2xl p-5 hover:border-red-500/50 transition-colors">
                <div className="flex items-center gap-3 mb-3">
                  <div className="p-2 bg-slate-700 rounded-lg text-slate-300">{b.icon}</div>
                  <h4 className="font-bold text-sm">{b.name}</h4>
                </div>
                <p className="text-xs text-slate-400 mb-4">{b.contexts.length} predefined contexts</p>
                <button onClick={() => handleInjectBundle(b)} className="w-full py-2 bg-slate-700 hover:bg-slate-600 text-white rounded-xl text-xs font-bold transition-colors">
                  Inject Bundle
                </button>
              </div>
            ))}
