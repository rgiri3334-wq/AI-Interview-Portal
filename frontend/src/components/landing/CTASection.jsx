    ...
    <section className="py-24 px-6 flex justify-center relative z-10 bg-white">
      ...
      >
        ...
        <h2 className="text-3xl md:text-5xl font-black mb-6 tracking-tight text-white relative z-10">
          Ready to Automate <span className="text-transparent bg-clip-text bg-gradient-to-r from-red-400 to-red-600">Hiring</span>?
        </h2>
        <p className="text-slate-400 text-lg mb-12 max-w-2xl mx-auto font-medium relative z-10 leading-relaxed">
          Access enterprise root controls, manage candidate pipelines, and review system telemetry in real-time. Join the future of autonomous interviewing.
        </p>
        ...
        <motion.button 
          ...
          whileTap={{ scale: 0.98 }}
          className="bg-red-600 text-white font-bold py-4 px-10 rounded-full text-base transition-all flex items-center gap-3 uppercase tracking-wide shadow-lg relative z-10" 
          onClick={() => navigate('/admin')}
        >
          Deploy AI Engine <ArrowRight size={18} />
        </motion.button>
      </motion.div>
    </section>
  );
