...
try:
    ...
    GROQ_OK = False
    logger.warning(
        "groq SDK not installed. Whisper via Groq unavailable. pip install groq")
...
async def transcribe_audio(
    ...
    logger.error(
        "All Whisper services unavailable. Frontend should use browser SpeechRecognition.")
    return _empty_result("all_services_down")


async def _transcribe_groq(
        audio_bytes: bytes, filename: str, language: str) -> dict:
    import time
    t0 = time.monotonic()
    ...
    logger.info(f"Groq Whisper transcribed {len(audio_bytes) /
                                            1024:.1f}KB in {elapsed_ms}ms: '{transcript[:60]}...'")
