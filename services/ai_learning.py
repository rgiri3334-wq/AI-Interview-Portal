...
def save_lesson(mistake: str, rule: str, lesson_learned: str = "", future_improvement: str = "") -> None:
...
def get_recent_lessons(limit: int = 3) -> List[str]:
    ...
    with _db_lock:
        ...
        recent = data[-limit:]
        return [entry.get("new_rule", "") for entry in recent if entry.get("new_rule")]
...
def get_all_stats() -> Dict:
    ...
    active_rules = [d.get("new_rule", "") for d in active_entries if d.get("new_rule")]
